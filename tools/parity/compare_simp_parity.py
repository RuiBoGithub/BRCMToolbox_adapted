#!/usr/bin/env python3
"""Compare independently generated MATLAB and Python _simp.idf artifacts."""
...
def main() -> int:
    ...
    parser.add_argument("--root", type=Path, default=REPOSITORY_ROOT / "outputs/parity/simp")
    ...
    expected_source = (REPOSITORY_ROOT / "tests/fixtures/energyplus/_simp.idf").resolve()
    expected_hash = sha256_file(expected_source)
    same_input = bool(matlab_manifest and python_manifest and matlab_hash == python_hash == expected_hash and
                      matlab_manifest.get("source_idf") == python_manifest.get("source_idf") == "_simp.idf" and
                      Path(matlab_manifest.get("source_idf_path", "")).resolve() == expected_source and
                      Path(python_manifest.get("source_idf_path", "")).resolve() == expected_source)
    report: dict[str, object] = {"source_idf": "_simp.idf", "matlab_sha256": matlab_hash,
    ...
    lines = [f"Source IDF: _simp.idf", f"MATLAB SHA256: {matlab_hash}", f"Python SHA256: {python_hash}",
             f"Same input file: {status(same_input)}", "", "# `_simp.idf` Operational Parity", "",
