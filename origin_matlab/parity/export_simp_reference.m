...
% Trace the one IDF value whose original converter deliberately quantizes
% with bare num2str while assembling the construction sheet.
[extended_objects, ~] = getExtendedIDFObjects(idf_file);
normalized_objects = convertIDFObjects(extended_objects);
material_index = find(strcmp({normalized_objects.materials.identifier}, ...
    'C5 - 4 IN HW CONCRETE'), 1);
assert(~isempty(material_index), 'export_simp_reference:TraceMaterial', ...
    'Could not trace material C5 - 4 IN HW CONCRETE.');
normalized_thickness = normalized_objects.materials(material_index).thickness;
generated_thickness = num2str(normalized_thickness);
...
construction_index = find(strcmp({B.thermal_model_data.constructions.description}, ...
    'FLOOR'), 1);
assert(~isempty(construction_index), 'export_simp_reference:TraceConstruction', ...
    'Could not trace converted FLOOR construction.');
reloaded_thickness = str2double( ...
    B.thermal_model_data.constructions(construction_index).thickness{1});
...
thickness_trace = struct( ...
    'source_idf', struct('object', 'Material', 'name', 'C5 - 4 IN HW CONCRETE', ...
        'field', 'Thickness', 'line', 223, 'value', 0.1014984, 'units', 'm'), ...
    'parser_text', '0.1014984', ...
    'normalized_value', normalized_thickness, ...
    'generated_table_value', generated_thickness, ...
    'serialized_value', generated_thickness, ...
    'reloaded_value', reloaded_thickness, ...
    'effective_model_value', reloaded_thickness, ...
    'serialization_format', 'num2str(value) default precision', ...
    'classification', 'workflow quantization');
write_json(fullfile(output_directory, 'thickness_trace.json'), thickness_trace);
manifest = struct('format', 'brcm-simp-matlab-reference', 'format_version', 1, ...
    ...
    'number_of_steps', N, 'thickness_trace', 'thickness_trace.json', ...
